...
export default function* getAccountFavoriteTv(action) {
  try {
    // execute query
    const response = yield call(accountFavoriteTvQuery, action.payload);
    ...
    if (error) {
      ...
      console.log('[AccountFavoriteTvService] Service call '
          + `${GET_ACCOUNT_DETAILS_SERVICE_PATH} KO`, error);
      yield put({
        type: ACCOUNT_FAVORITE_TV_REQUEST_FAILURE,
        error,
      });
    } else {
      ...
      yield put({
        type: ACCOUNT_FAVORITE_TV_REQUEST_SUCCESS,
        accountDetails: data,
      });
    }
  } catch (error) {
